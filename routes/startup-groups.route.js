...
async function getStartupGroupsForUser(req, res, next) {
    try {
        const errors = validationResult(req);
        if (!errors.isEmpty()) {
            return res.status(422).json({ errorCode: 422, errors: errors.array() });
        ...
        const filterParams = ["name"];
        const filter = lodash.pick(req.query, filterParams);
        const startupGroups = await startupGroupsController.getStartupGroupsForUser(req.userId, filter, pagination);
        res.status(200).json({
            success: true,
            data: startupGroups,
        });
...
async function joinStartupGroup(req, res, next) {
    try {
        const errors = validationResult(req);
        if (!errors.isEmpty()) {
            return res.status(422).json({ errorCode: 422, errors: errors.array() });
        ...
        const groupId = req.params.groupId;
        await startupGroupsController.joinStartupGroup(req.userId, groupId);
        res.status(200).json({
            success: true,
        });
    } catch(err) {
...
async function leaveStartupGroup(req, res, next) {
    try {
        const errors = validationResult(req);
        if (!errors.isEmpty()) {
            return res.status(422).json({ errorCode: 422, errors: errors.array() });
        ...
        const groupId = req.params.groupId;
        await startupGroupsController.leaveStartupGroup(req.userId, groupId);
        res.status(200).json({
            success: true,
        });
    } catch(err) {
